  ...
  function _bulletLine(label, txt) {
    return `
      <div style="display:flex;gap:8px;align-items:flex-start;margin-top:6px;">
        <div style="width:18px;flex:0 0 18px;text-align:left;">${label}</div>
        <div style="flex:1;min-width:0;">${_wrapTextHTML(txt)}</div>
      </div>
    `;
  }
    ...
    if (data.type === '신입') {
      bodyHtml += `
        ...
        <li style="margin-bottom:12px;">
          ...
          <div style="margin-top:6px;">● 필요역량 : ${_wrapTextHTML(f.competency?.needed||'')}</div>
          <div>● 본인의 역량 보유 수준 : ${Number(f.competency?.score||0)} 점 / 10점 만점</div>
          <div>● 필요 역량을 갖추기 위한 과정(노력) :</div>
          ${_wrapTextHTML(f.competency?.effort||'')}
        </li>

        <li style="margin-bottom:12px;">
          타인이 인정(칭찬)하는 본인 성격(성향)상의 장점과 그 이유를 기술해주십시오.
          ${_wrapTextHTML(f.personalityStrength||'')}
        </li>
        ...
        <li style="margin-bottom:12px;">
          안랩 외에 현재 최종면접이 진행중이거나 합격한 회사가 있습니까?
          ${_wrapTextHTML(f.otherOffers||'')}
        </li>
      ...
      bodyHtml += `
        <li style="margin-bottom:12px;">
          직장생활에서의 성공에 대해서 정의해 보십시오.
          ${_wrapTextHTML(f.successDef||'')}
        </li>

        <li style="margin-bottom:12px;">
          본인 성격의 장/단점에 대해 각각 간략하게 기입해 주십시오.
          <div>#장점 - ${_wrapTextHTML(f.pros||'')}</div>
          <div>#단점 - ${_wrapTextHTML(f.cons||'')}</div>
        </li>
        ...
        <li style="margin-bottom:12px;">
          나를 표현하는 단어 3가지를 기입해 주십시오.
          ${_wrapTextHTML(f.selfWords||'')}
        </li>

        <li style="margin-bottom:12px;">
          가장 큰 성취를 했던 경험과 그 때 본인이 맡았던 역할을 기술해 주십시오.
          ${_wrapTextHTML(f.bigAchievement||'')}
        </li>
